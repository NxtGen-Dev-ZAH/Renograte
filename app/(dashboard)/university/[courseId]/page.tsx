    ...
    const fetchCourseDetails = async () => {
      try {
        ...
        if (data.videos && data.videos.length > 0) {
          ...
          const firstUncompletedVideo = data.videos.find((video: CourseVideo) => !video.completed);
  ...
  const updateVideoProgress = async (videoId: string, watchedSeconds: number, lastPosition: number, completed: boolean) => {
    try {
      ...
      if (completed && !activeVideo?.completed) {
        ...
        setOverallProgress(Math.round(((completedVideos + 1) / totalVideos) * 100));
  ...
  if (!course || !activeVideo) {
    return (
      <div className="text-center py-10">
        ...
        <p className="text-muted-foreground mt-2">The requested course could not be loaded.</p>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2">
          <Card className="overflow-hidden">
            ...
            <CardContent>
              <p className="text-sm text-muted-foreground mb-4">{activeVideo.description}</p>
                ...
                {course.videos.map((video, index) => (
                  <div 
                    ...
                    className={`flex items-center gap-2 p-2 rounded-md cursor-pointer ${
                      activeVideo.id === video.id ? 'bg-secondary' : 'hover:bg-secondary/50'
                    }`}
