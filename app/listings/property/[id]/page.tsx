...
export default function PropertyDetailPage() {
  ...
  const calculateRenovationDetails = (property: Property) => {
    ...
    const profitMargin = 0.30; // 30% profit margin on renovation cost
    const arv = property.ListPrice + renovationAllowance + (renovationAllowance * profitMargin);
    
    return {
      renovationAllowance: Math.round(renovationAllowance),
      afterRenovationValue: Math.round(arv)
    };
  };
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] flex flex-col">
      ...
      {/* Back to listings */}
      <Link href="/listings" className="inline-flex items-center text-blue-600 hover:text-blue-800 mb-6">
        <ArrowLeft className="mr-2 h-4 w-4" /> Back to listings
      </Link>
