...
function PropertiesPage() {
  ...
  const generatePropertyTitle = (property: ApiPropertyResponse): string => {
    ...
    const adjectives =
      propertyAdjectives[propertyType as PropertyType] ||
      propertyAdjectives[PropertyType.Residential];
    ...
    if (propertyType === PropertyType.Land) {
      ...
      const yearText =
        property.YearBuilt && property.YearBuilt > currentYear - 10
          ? ` (${property.YearBuilt})`
          : "";
    ...
    if (data && data.value) {
      const mappedProperties = data.value.map((item: ApiPropertyResponse) => {
        ...
        const mainImage = item.Media?.length
          ? item.Media.find((m) => m.Order === 1)?.MediaURL ||
      ...
      if (isLoadingMore) {
        setProperties((prev) => {
          ...
          const uniqueNewProperties = mappedProperties.filter(
            (property) => !existingIds.has(property.id)
          );

          console.log(
            `Found ${mappedProperties.length - uniqueNewProperties.length} duplicate properties`
          );
          ...
          if (uniqueNewProperties.length === 0 && data["@odata.nextLink"]) {
            console.log(
              "All properties were duplicates, automatically loading next page..."
            );
            ...
            setTimeout(() => {
              ...
              console.log(
                `Automatically loading page ${pageToLoad + 1}, skipping ${skipCount} items`
              );
              buildQuery(skipCount);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <TooltipContent className="max-w-md p-4 text-left">
              <div className="space-y-2">
                ...
                <p className="text-sm text-gray-700">
                  ...
                  <span className="font-semibold text-gray-900">envision</span>{" "}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={setRenovationFilter}
            >
              <SelectTrigger className="w-full">
                <SelectValue placeholder="Any" />
              </SelectTrigger>
      ...
      <div className="mt-16 text-center bg-gray-50 p-8 rounded-lg">
        ...
        <p className="text-gray-600 mb-6 max-w-2xl mx-auto">
          Our team of experts can help you find properties with renovation
          potential that match your specific requirements.
        </p>
        <Button
          ...
          onClick={() => {
            ...
            if (isAuthenticated || hasToken) {
              ...
              router.push(
                `/login?returnUrl=${encodeURIComponent("/directories?category=agents")}`
              );
            }
          }}
        >
